...
########################################################################################
# Table2  predicting realized volatility with ivol dictionaries D0 for Earnings calls 
########################################################################################

## Load data
meta <- read.csv("data/output/volatility_signals/regression_ttlvol.csv") 
...
writeLines(reg_summary, "data/output/predicting_rv/results.txt")
...
write.csv(coefficients_df, file='data/output/predicting_rv/coeffs.csv', row.names=FALSE)
