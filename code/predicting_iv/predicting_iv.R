...
###################################################################################
# Table  predicting implied volatility with ivol dictionaries D1 for Earnings calls
###################################################################################

## Load data
meta <- read.csv("data/output/volatility_signals/regression_iv.csv") 
...
writeLines(reg_summary, "data/output/predicting_iv/results.txt")

# save coefficients of predicting
coefficients_df <- data.frame(Model = character(0), 
...
write.csv(coefficients_df, file='data/output/predicting_iv/coeffs.csv', row.names=FALSE)