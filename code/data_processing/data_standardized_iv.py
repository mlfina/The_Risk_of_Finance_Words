...
def get_rv_iv(meta, days_before, days_after):
    ...
    if selected_data.empty:
        ...
        num_rows_before = max(0, 302 - center_index)
        num_rows_after = max(0, 50 - (len(selected_data) - 1 - center_index))
        ...
        selected_data['mean_iv'] = selected_data['ivolc'].rolling(252, min_periods=2).mean().shift(1)
        selected_data['temp_iv'] = selected_data['ivolc'] / selected_data['mean_iv']
        selected_data['diff_iv'] = selected_data['temp_iv'].diff()
        iv_data = selected_data['diff_iv'][282:323].to_list()
...
if __name__ == '__main__':
    ...
    for index, meta_temp in meta.iterrows():
        iv_data_temp= get_rv_iv(meta_temp, 302, 50)
    ...
    ## save data 
    regression_iv.to_csv(r'data/input/regression_standardized_iv.csv', index=False)
